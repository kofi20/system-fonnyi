...
{
    protected $tt, $my_class, $exam, $year, $dorm, $user;

    public function __construct(TimeTableRepo $tt, MyClassRepo $mc,  UserRepo $user, ExamRepo $exam, DormRepo $dorm)
    {
        $this->tt = $tt;
        $this->my_class = $mc;
        $this->exam = $exam;
    ...
    public function index()
    {
        ...
        //Fetching data
        $classController = new MyClassController($this->my_class, $this->$user);
        $classData = $classController->index();
        
        $sectionController = new SectionController($this->my_class, $this->$user);
        $sectionData = $classController->index();
        
        $subjectController = new SubjectController($this->my_class, $this->$user);
        $subjectData = $subjectController->index();
        
        $dormController = new DormController($this->dorm);
        $dormData = $dormController->index();

        // Merge
        $d = array_merge($d, $classData, $sectionData, $subjectData, $dormData);
