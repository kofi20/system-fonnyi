...
class PromotionController extends Controller
{
    ...
    public function promotion(Request $request)
    {
        ...
        if ($request->filled(['fc', 'fs', 'tc', 'ts'])) {
            $d['selected'] = true;
            $d['fc'] = $request->input('fc');
            $d['fs'] = $request->input('fs');
            $d['tc'] = $request->input('tc');
            $d['ts'] = $request->input('ts');
            $d['students'] = $sts = $this->student->getRecord([
                'my_class_id' => $d['fc'],
                'section_id' => $d['fs'],
                'session' => $d['old_year']
            ])->get();
        ...
        // Render the view directly with the promotion data
        return View::make('pages.support_team.students.promotion.index', $d);
    }
    ...
    {
        // Instead of using $fc, $fs, $tc, $ts directly, you can get them from the Request object
        $fc = $request->input('fc');
        $fs = $request->input('fs');
        $tc = $request->input('tc');
        $ts = $request->input('ts');
